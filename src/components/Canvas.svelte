<script>
  ...
  $: focusBlob = $canvasState.findIndex((x) => x.focussed === true);
</script>
...
<svg viewBox="-{ margin } -{ margin } { cols * areaConfig.size + 2 * margin} { height + 2 * margin }">
  ...
  <g class="editor">
    ...
    <Blob 
      { ...$canvasState[focusBlob] }
      { ...areaConfig }
      canvasHeight={ height }
    />
    <g class="button" transform="translate({ cols * areaConfig.size - 50 } { 0 })" on:click={ () => $canvasState[focusBlob].focussed = false } >
